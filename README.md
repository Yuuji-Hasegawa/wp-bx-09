...
- WordPress テーマ、[BX-09](https://demo.bbns.jp/bx-09/)のリポジトリです
- 当テーマで使用している CSS、JS、画像については[html-bx-09](https://github.com/Yuuji-Hasegawa/html-bx-09)をお使いいただくか、別途用意の上加工してください
...
4. 使用するテーマを bx-09 に変更し、パーマリンクを変更してください
...
1. テーマ名を変更する場合は、`.gitignore`内のテーマ名部分、`wp-bx-09/style.css`を編集してください
2. 各サイトの設計に合わせ、`wp-bx-09/lib/breadcrumb.php`、`wp-bx-09/lib/json-ld.php`を編集ください
3. 固有名詞や favicon、フォントの preload は、`wp-bx-09/lib/setting.json`、、`wp-bx-09/lib/head.php`、`wp-bx-09/pwa/manifest.json`、`wp-bx-09/pwa/sw.js`を編集ください
4. `page-hoge.php`の`hoge`は slug に合わせて編集ください。固定ページの中身を`the_content`で流し込む場合は設定に合わせてください
5. jQuery や reCAPTCHA の読み込みを`wp-bx-09/lib/script-load.php`で制御しています。適宜編集ください
